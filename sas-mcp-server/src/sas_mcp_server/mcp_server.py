...
from .config import get_viya_auth
...
class AuthMiddleware(Middleware):
    async def on_call_tool(self, ctx: MiddlewareContext, call_next):
        ...
        if bearer_token:
            ...
            viya_access_info = await get_viya_auth().load_access_token(jwt)
            if viya_access_info:
...
mcp = FastMCP("SAS Viya Execution MCP Server", auth=get_viya_auth())

...
async def refresh_viya_token(client=None):
    """Uses VIYA_REFRESH_TOKEN to get a new access token, updating .env and the current client."""
    logger.info("Refreshing SAS Viya access token using refresh token...")
    refresh_token = os.getenv("VIYA_REFRESH_TOKEN")
    if not refresh_token:
        logger.error("No VIYA_REFRESH_TOKEN found in environment. Cannot refresh!")
        raise ValueError("VIYA_REFRESH_TOKEN is not set.")
    ...
    async with httpx.AsyncClient(verify=SSL_VERIFY) as temp_client:
        r = await temp_client.post(token_url, auth=auth, data=data)
        if r.status_code != 200:
            logger.error(f"Token refresh API failed: {r.text}")
            r.raise_for_status()
        ...
        # Save to .env file in the project root
        env_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".env"))
        try:
            dotenv.set_key(env_path, "VIYA_ACCESS_TOKEN", new_access)
            dotenv.set_key(env_path, "VIYA_REFRESH_TOKEN", new_refresh)
        except Exception as e:
            logger.warning(f"Could not write to .env file: {e}")
        
        # Update process environment variables
        os.environ["VIYA_ACCESS_TOKEN"] = new_access
        os.environ["VIYA_REFRESH_TOKEN"] = new_refresh
        
        if client:
            client.headers["Authorization"] = f"Bearer {new_access}"
            
        logger.info("Successfully refreshed access token and updated .env!")
        return new_access
...
def _make_client(token):
    ...
    # Ensure token is loaded dynamically if empty or stale
    current_token = os.getenv("VIYA_ACCESS_TOKEN", token)
    if not current_token.startswith("Bearer "):
        current_token = f"Bearer {current_token}"
        
    headers = {"Authorization": current_token}
    client = httpx.AsyncClient(headers=headers, verify=SSL_VERIFY, timeout=300.0)
    
    # Monkey-patch the request method to auto-refresh on 401 Unauthorized
    original_request = client.request
    
    async def _auto_refresh_request(method, url, **kwargs):
        resp = await original_request(method, url, **kwargs)
        if resp.status_code == 401:
            logger.warning(f"SAS API returned 401 Unauthorized for {url}. Attempting to refresh token...")
            try:
                await refresh_viya_token(client)
                # Retry the request with the new headers automatically applied to the client
                logger.info("Retrying the failed request with new token...")
                resp = await original_request(method, url, **kwargs)
            except Exception as e:
                logger.error(f"Failed to recover from 401: {e}")
        return resp
        
    client.request = _auto_refresh_request
    return client
